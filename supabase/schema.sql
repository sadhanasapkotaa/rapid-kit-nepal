...
-- Dashboard first, then these policies allow public read + authenticated write.
...
create policy "auth write product media" on storage.objects for insert to authenticated
  with check (bucket_id in ('product-images','product-videos'));
create policy "auth update product media" on storage.objects for update to authenticated
  using (bucket_id in ('product-images','product-videos'));
create policy "auth delete product media" on storage.objects for delete to authenticated
  using (bucket_id in ('product-images','product-videos'));
